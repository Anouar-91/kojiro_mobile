...
export function mapProfileToUser(profile: DbProfile): User {
  ...
  return {
    ...
    stats: {
      ...
      averageDefensiveRating:
        stats.averageDefensiveRating ?? DEFAULT_STATS.averageDefensiveRating,
...
export function mapDbMatchToMatch(row: DbMatch): Match {
  const attendees: MatchAttendee[] = (row.match_attendees ?? []).map((a) => ({
    ...
    guestPosition: (a.guest_position as MatchAttendee['guestPosition']) ?? undefined,
