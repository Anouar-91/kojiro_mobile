-- Migration 040 : composition limitée aux joueurs présents
-- Exécuter dans Supabase SQL Editor

CREATE OR REPLACE FUNCTION public.clear_match_lineup_for_attendee(
  p_match_id UUID,
  p_user_id UUID,
  p_attendee_id UUID
)
...
BEGIN
  ...
  ELSIF p_attendee_id IS NOT NULL THEN
    DELETE FROM public.match_lineups
    WHERE match_id = p_match_id AND attendee_id = p_attendee_id;
  END IF;
END;
$$;
...
BEGIN
  IF TG_OP = 'DELETE' THEN
    PERFORM public.clear_match_lineup_for_attendee(OLD.match_id, OLD.user_id, OLD.id);
  ...
  IF OLD.status = 'present' AND NEW.status IS DISTINCT FROM 'present' THEN
    PERFORM public.clear_match_lineup_for_attendee(NEW.match_id, NEW.user_id, NEW.id);
  ...
  LOOP
    ...
    IF v_user_id IS NOT NULL THEN
      IF v_user_id = ANY(v_seen_users) THEN
        CONTINUE;
      END IF;
      IF NOT public.is_present_match_attendee(p_match_id, v_user_id) THEN
        CONTINUE;
      END IF;
...
GRANT EXECUTE ON FUNCTION public.clear_match_lineup_for_attendee(UUID, UUID, UUID) TO authenticated;
