-- Migration 037 : note fair-play /5 (auto-déclaration → capitaine → orga, défaut 4)

ALTER TABLE public.match_stat_entries
  ADD COLUMN IF NOT EXISTS self_fair_play DECIMAL(2,1),
  ADD COLUMN IF NOT EXISTS captain_fair_play DECIMAL(2,1);

DROP FUNCTION IF EXISTS public.submit_my_match_stats(UUID, INT, INT, UUID, DECIMAL);
...
GRANT EXECUTE ON FUNCTION public.submit_my_match_stats(UUID, INT, INT, UUID, DECIMAL, DECIMAL) TO authenticated;
