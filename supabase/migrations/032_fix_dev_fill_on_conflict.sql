-- Migration 032 : corriger dev_fill_match_attendees (ON CONFLICT après index partiel 023)
...
BEGIN
  ...
  LOOP
    IF EXISTS (
      SELECT 1 FROM public.match_attendees ma
      WHERE ma.match_id = p_match_id AND ma.user_id = v_profile.id
    ) THEN
      UPDATE public.match_attendees
      SET status = 'present'
      WHERE match_id = p_match_id AND user_id = v_profile.id;
    ELSE
