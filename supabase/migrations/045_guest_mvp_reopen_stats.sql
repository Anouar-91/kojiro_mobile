-- Migration 045 : vote MVP pour invités + réouverture des stats par l'organisateur

ALTER TABLE public.match_mvp_votes
  ALTER COLUMN voted_for_id DROP NOT NULL;

ALTER TABLE public.match_mvp_votes
  ADD COLUMN IF NOT EXISTS voted_for_attendee_id UUID REFERENCES public.match_attendees(id) ON DELETE CASCADE;

ALTER TABLE public.match_mvp_votes
  DROP CONSTRAINT IF EXISTS match_mvp_votes_target_chk;

ALTER TABLE public.match_mvp_votes
  ADD CONSTRAINT match_mvp_votes_target_chk
  CHECK (
    (voted_for_id IS NOT NULL AND voted_for_attendee_id IS NULL)
    OR (voted_for_id IS NULL AND voted_for_attendee_id IS NOT NULL)
  );

CREATE OR REPLACE FUNCTION public.record_mvp_vote(
  p_match_id UUID,
  p_voter_id UUID,
...
DECLARE
  ...
  v_mvp_team TEXT;
BEGIN
  IF (p_mvp_user_id IS NULL AND p_mvp_attendee_id IS NULL)
    OR (p_mvp_user_id IS NOT NULL AND p_mvp_attendee_id IS NOT NULL) THEN
    RAISE EXCEPTION 'Vote MVP invalide';
  END IF;
  ...
  IF p_mvp_user_id IS NOT NULL AND p_mvp_user_id = p_voter_id THEN
  ...
  ELSE
    SELECT mse.team_side INTO v_mvp_team
    FROM public.match_stat_entries mse
    WHERE mse.match_id = p_match_id AND mse.attendee_id = p_mvp_attendee_id;
  END IF;
  ...
  INSERT INTO public.match_mvp_votes (match_id, voter_id, voted_for_id, voted_for_attendee_id)
  VALUES (p_match_id, p_voter_id, p_mvp_user_id, p_mvp_attendee_id)
  ON CONFLICT (match_id, voter_id)
  DO UPDATE SET
    voted_for_id = EXCLUDED.voted_for_id,
    voted_for_attendee_id = EXCLUDED.voted_for_attendee_id,
    created_at = now();
END;
$$;

DROP FUNCTION IF EXISTS public.submit_my_match_stats(UUID, INT, INT, UUID, DECIMAL, DECIMAL);
...
DROP FUNCTION IF EXISTS public.captain_save_team_stats(UUID, TEXT, JSONB, UUID);
...
DROP FUNCTION IF EXISTS public.finalize_match_stats(UUID, JSONB, UUID);
...
GRANT EXECUTE ON FUNCTION public.record_mvp_vote(UUID, UUID, UUID, UUID) TO authenticated;
GRANT EXECUTE ON FUNCTION public.submit_my_match_stats(UUID, INT, INT, UUID, UUID, DECIMAL, DECIMAL) TO authenticated;
