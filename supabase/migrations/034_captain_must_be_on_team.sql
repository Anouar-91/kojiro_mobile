-- Migration 034 : le capitaine doit être dans l'équipe correspondante
...
BEGIN
  ...
  IF p_captain_a_id IS NOT NULL AND NOT EXISTS (
    ...
    RAISE EXCEPTION 'Le capitaine A doit être dans l''équipe A';
  END IF;
  ...
  IF p_captain_b_id IS NOT NULL AND NOT EXISTS (
    ...
    RAISE EXCEPTION 'Le capitaine B doit être dans l''équipe B';
  END IF;
...
BEGIN
  ...
  SET
    captain_a_id = CASE
      WHEN captain_a_id IS NOT NULL AND NOT EXISTS (
        SELECT 1 FROM public.match_lineups
        WHERE match_id = p_match_id AND team_side = 'A' AND user_id = captain_a_id
      ) THEN NULL
      ELSE captain_a_id
    END,
    captain_b_id = CASE
      WHEN captain_b_id IS NOT NULL AND NOT EXISTS (
        SELECT 1 FROM public.match_lineups
        WHERE match_id = p_match_id AND team_side = 'B' AND user_id = captain_b_id
      ) THEN NULL
      ELSE captain_b_id
    END,
    updated_at = now()
